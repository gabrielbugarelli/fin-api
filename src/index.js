...
async function getBalance (statement) {
  const balance = await statement.reduce((acc, operation) => {
...
app.post('/account', (request, response) => {
    ...
    return response.status(201).json({name, cpf}).send()
})
...
app.get('/account/all', (request, response) => {
  return response.json({ customers }).status(200).send()
})

app.get('/balance', verifyExistsAccountCPF, async (request, response) => {
  const { customer } = request;
